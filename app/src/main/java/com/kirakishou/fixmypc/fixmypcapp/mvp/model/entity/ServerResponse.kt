...
sealed class ServerResponse<out R> {
    ...
    data class ServerError<out T>(val errorCode: ErrorCode.Remote): ServerResponse<T>()
    data class LocalError<out T>(val errorCode: ErrorCode.Local): ServerResponse<T>()
